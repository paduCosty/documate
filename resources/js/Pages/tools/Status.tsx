...
const operationLabels: Record<string, { success: string; subtitle: (f: any) => string; download: string; again: string; processingMsg: string; }> = {
    ...
    'excel-to-pdf': { success: "Conversion completed successfully!", subtitle: (f) => { const c = f.metadata?.converted_count; return c && c > 1 ? c + ' spreadsheets converted to PDF' : "Your Excel spreadsheet has been converted"; }, download: "Download PDF", again: "Convert another file", processingMsg: "Converting to PDF..." },
    'pdf-to-jpg':   { success: "Conversion completed successfully!", subtitle: (f) => { const c = f.metadata?.page_count; return c ? c + ' page' + (c > 1 ? 's' : '') + ' converted to JPG' : 'Your PDF has been converted to images'; }, download: "Download ZIP", again: "Convert another file", processingMsg: "Converting pages to JPG..." },
    'ppt-to-pdf':   { success: "Conversion completed successfully!", subtitle: (f) => { const c = f.metadata?.converted_count; return c && c > 1 ? c + ' presentations converted to PDF' : 'Your presentation has been converted'; }, download: "Download PDF", again: "Convert another file", processingMsg: "Converting to PDF..." },
};
